...
class ArenaTest {
    ...
    @Test
    void setGraph() {
        DWGraph_DS graph = Arena.json2Graph("{\"Edges\":[{\"src\":0,\"w\":1.0,\"dest\":1},{\"src\":0,\"w\":1.4,\"dest\":2},{\"src\":0,\"w\":0.9,\"dest\":3},{\"src\":2,\"w\":1.35,\"dest\":3},{\"src\":3,\"w\":1.35,\"dest\":2},{\"src\":2,\"w\":1.35,\"dest\":0}],\"Nodes\":[{\"pos\":\"35.212,32.1062,0.0\",\"id\":0},{\"pos\":\"35.211,32.1050,0.0\",\"id\":1},{\"pos\":\"35.2,32.1,0.0\",\"id\":2},{\"pos\":\"35.211,32.102,0.0\",\"id\":3}]}");
        Arena testArena = new Arena();
        testArena.setGraph(graph);

        assertEquals(graph , testArena.getGraph());
    }


    @Test
    void json2Pokemons() {
        String pokemons = "{\"Pokemons\":[{\"Pokemon\":{\"value\":5.0,\"type\":-1,\"pos\":\"35.192831596416106,32.10779138402466,0.0\"}},{\"Pokemon\":{\"value\":8.0,\"type\":1,\"pos\":\"35.18752225008876,32.10379910146876,0.0\"}},{\"Pokemon\":{\"value\":13.0,\"type\":1,\"pos\":\"35.2036278051956,32.10225131838408,0.0\"}}]}";
        ArrayList<CL_Pokemon> Jpokemons = Arena.json2Pokemons(pokemons);
        ArrayList<CL_Pokemon> Pokemons = geTPokemons(1);
        for(int i = 0;i<3;i++){
            assertEquals(Jpokemons.get(i).getValue(),Pokemons.get(i).getValue());
            assertEquals(Jpokemons.get(i).getType(),Pokemons.get(i).getType());
        }
        Pokemons = geTPokemons(0);
        for(int i = 0;i<3;i++){
            assertNotEquals(Jpokemons.get(i).getValue(),Pokemons.get(i).getValue());
            assertEquals(Jpokemons.get(i).getType(),Pokemons.get(i).getType());
        }
    }
    ...
    @Test
    void json2Graph() {
        String testString = "{\"Edges\":[{\"src\":0,\"w\":1.0,\"dest\":1},{\"src\":0,\"w\":1.4,\"dest\":2},{\"src\":0,\"w\":0.9,\"dest\":3},{\"src\":2,\"w\":1.35,\"dest\":3},{\"src\":3,\"w\":1.35,\"dest\":2},{\"src\":2,\"w\":1.35,\"dest\":0}],\"Nodes\":[{\"pos\":\"35.212,32.1062,0.0\",\"id\":0},{\"pos\":\"35.211,32.1050,0.0\",\"id\":1},{\"pos\":\"35.2,32.1,0.0\",\"id\":2},{\"pos\":\"35.211,32.102,0.0\",\"id\":3}]}";
        DWGraph_DS graph = simpleGraph();
        DWGraph_DS Jgraph = Arena.json2Graph(testString);
        setLocation(Jgraph, graph);
        assertEquals(Jgraph, graph);

        graph.removeNode(3);
        assertNotEquals(Jgraph, graph);
    ...
    private DWGraph_DS simpleGraph() {
        ...
        graph.connect(0, 3, 0.9);
        graph.connect(2, 3, 1.35);
        graph.connect(3, 2, 1.35);
        graph.connect(2, 0, 1.35);

        return graph;
    }

    //test method
    private void setLocation(DWGraph_DS Jgraph, DWGraph_DS graph) {
        location = new GeoLocation[4];
        Random r = new Random();
        double randomValue = 25 + (35 - 25) * r.nextDouble();

        for (int i = 0; i < location.length; i++) {
            this.location[i] = new GeoLocation(randomValue, randomValue, randomValue);
            graph.getNode(i).setLocation(location[i]);
            Jgraph.getNode(i).setLocation(location[i]);
        }
    }

    private ArrayList<CL_Pokemon> geTPokemons(int flag){
        Random r = new Random();
        double randomValue = 25 + (35 - 25) * r.nextDouble();
        ArrayList<CL_Pokemon> pokemons = new ArrayList<>();
        if(flag == 1) {
            CL_Pokemon pokemon = new CL_Pokemon(new Point3D(randomValue, randomValue, randomValue), -1, 5, 0, null);
            pokemons.add(pokemon);

            pokemon = new CL_Pokemon(new Point3D(randomValue, randomValue, randomValue), 1, 8, 0, null);
            pokemons.add(pokemon);

            pokemon = new CL_Pokemon(new Point3D(randomValue, randomValue, randomValue), 1, 13, 0, null);
            pokemons.add(pokemon);
        ...
        CL_Pokemon pokemon = new CL_Pokemon(new Point3D(randomValue, randomValue, randomValue), -1, 4.9, 0, null);
        pokemons.add(pokemon);

        pokemon = new CL_Pokemon(new Point3D(randomValue, randomValue, randomValue), 1, 7.9, 0, null);
        pokemons.add(pokemon);

        pokemon = new CL_Pokemon(new Point3D(randomValue, randomValue, randomValue), 1, 12.9, 0, null);
        pokemons.add(pokemon);
